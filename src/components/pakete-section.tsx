...
const tierBullets: Record<string, string[]> = {
  standard: ["Preisbewusst", "Flexibles Zeitfenster", "Für planbare Termine"],
  plus: ["Meistgewählt", "Schnelle Rückmeldung", "Balance aus Preis und Tempo"],
  premium: ["Priorisierte Planung", "Für dringende Aufträge", "Besonders kurze Vorlaufzeit"],
};
      ...
      <Container className="relative py-20">
        <Reveal>
          <div className="text-center">
            ...
            <h2 className="mt-4 text-2xl font-extrabold tracking-tight sm:text-3xl">Wählen Sie Ihr Paket</h2>
            <p className="mx-auto mt-2 max-w-xl text-sm text-slate-600 dark:text-slate-400">
              Standard, Plus oder Premium – je nach Dringlichkeit, Umfang und Budget.
            </p>
          </div>
        </Reveal>
        ...
        <Reveal className="mt-10">
          <div className="mx-auto grid max-w-4xl gap-6 sm:grid-cols-3">
            {[
              { key: "standard", label: "Standard", recommended: false },
              { key: "plus", label: "Plus", recommended: true },
              { key: "premium", label: "Premium", recommended: false },
            ].map((tier) => (
              <Link
                key={tier.key}
                href="/preise"
              ...
              >
                {tier.recommended ? (
                  <div className="absolute -top-3 left-1/2 inline-flex -translate-x-1/2 items-center gap-1 rounded-full bg-brand-600 px-3 py-1 text-xs font-bold text-white shadow-md">
                    <Sparkles className="h-3 w-3" />
                    Empfohlen
                  </div>
                ) : null}
                <div className="text-lg font-extrabold text-slate-950 dark:text-white">{tier.label}</div>
                ...
                <Button size="sm" variant={tier.recommended ? "primary" : "outline"} className="mt-6 w-full">
                  Preise ansehen
                </Button>
              </Link>
            ))}
